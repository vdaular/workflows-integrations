...
namespace Elsa.Integrations.GitHub.Activities.Organizations;
...
public class SearchOrganizationMembers : GitHubActivity
{
    ...
    protected override async ValueTask ExecuteAsync(ActivityExecutionContext context)
    {
        ...
        var request = new OrganizationMembersRequest();
        if (!string.IsNullOrEmpty(role))
        {
            request.Role = ParseRole(role);
        }

        var members = await client.Organization.Member.GetAll(organizationName, request, options);
